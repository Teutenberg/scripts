[string]$SqlInstanceName = "INST01"
[string]$SqlAgName = "AG"
[string]$SqlPrimaryNode = "SRV01"
[string[]]$SqlSecondaryNodes = ("SRV02,SRV21").Split(",")
[string]$SqlAdminUsername = "DC\USERNAME"

Import-Module dbatools, SqlServer, SqlServerDsc
...
if ($PrimaryHadr.ComputerNamePhysicalNetBIOS -ieq $env:COMPUTERNAME) {
    ...
    foreach ($db in $UserDatabases) {
        ...
        Backup-DbaDatabase -SqlInstance $SqlPrimary -Database $db.Name -BackupDirectory $SqlShareUnc -Type Full -FileCount 4 -CompressBackup $true
        Backup-DbaDatabase -SqlInstance $SqlPrimary -Database $db.Name -BackupDirectory $SqlShareUnc -Type Log
