SET NOCOUNT ON;
	...
	(
		SELECT [login].[name] AS [login_name]
			...
			,REPLACE(REPLACE((SELECT [account_name] AS [@account] FROM @login_info WHERE [group_sid]=[login].[sid] FOR XML PATH('A')), '<A ', '<'), '"/>', '">') AS [group_member]
			...
			,REPLACE(REPLACE((SELECT [db_name] AS [@database],[user_name] AS [@user],[role_name] AS [@role] FROM @database_roles WHERE [user_sid]=[login].[sid] ORDER BY [db_name],[user_name],[role_name] FOR XML PATH('A')), '<A ', '<'), '"/>', '">') AS [database_roles]
	...
	SELECT * 
	FROM Dataset 
	WHERE [login_name] NOT LIKE 'NT AUTHORITY\SYSTEM' COLLATE Latin1_General_CI_AS
		AND [login_name] NOT LIKE 'NT SERVICE\MSSQL%' COLLATE Latin1_General_CI_AS
		AND [login_name] NOT LIKE 'NT SERVICE\SQL%' COLLATE Latin1_General_CI_AS
		AND ([login_name] NOT LIKE 'NT SERVICE\Winmgmt'  COLLATE Latin1_General_CI_AS OR [server_roles] IS NOT NULL)
		AND ([login_name] NOT LIKE 'NT SERVICE\ReportServer%'  COLLATE Latin1_General_CI_AS OR [server_roles] IS NOT NULL)
		AND ([login_name] NOT LIKE '##%##'  COLLATE Latin1_General_CI_AS OR [server_roles] IS NOT NULL)
	ORDER BY [login_name];